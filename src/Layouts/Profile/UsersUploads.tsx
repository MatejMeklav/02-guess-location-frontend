...
export default function UsersUploads() {
  ...
  for (var i = 0; i < myUploadsList.length; i++) {
    const obj: pbObject = {
      id: myUploadsList[i].id,
      image: myUploadsList[i].image,
    };
  ...
  const itemsList = myUploadsArray.map((item) => (
    <div className="upload-item" key={item.id}>
      ...
      <img
        ...
        src={require("../Images/react-icon-pen.png")}
        alt="pen-icon"
      ></img>
      <img
        className="x-icon"
    ...
    <>
      <div className="my-uploads-list">
        ...
        {deleteLocationClicked ? (
          <DeleteLocation id={deleteLocationId}></DeleteLocation>
        ) : (
          ""
        )}
      </div>
